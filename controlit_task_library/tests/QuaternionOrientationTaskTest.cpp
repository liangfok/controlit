...
#include <controlit/task_library/QuaternionOrientationTask.hpp>
...
class QuaternionOrientationTaskTest : public ::testing::Test
{
protected:
  virtual void SetUp()
  {
...
TEST_F(QuaternionOrientationTaskTest, WorldFrameTest)
{
  std::unique_ptr<controlit::Task> task(new controlit::task_library::QuaternionOrientationTask);
  ...
  qd.setIdentity();
  goalPos << qd.x(), qd.y(), qd.z(), qd.w();
  EXPECT_TRUE(p->set(goalPos));

  // Set the goal velocity parameter (TODO--implement tracking, not done yet)
  p = task->lookupParameter("goalVelocity");
  EXPECT_TRUE(p) << "Unable to get goalVelocity parameter.";
  Vector goalVel(3); goalVel.setZero();  // For a OrientationTask, the goal position as 3 dimensions: x, y, z
  EXPECT_TRUE(p->set(goalVel));

  // set kp parameter
  p = task->lookupParameter("kp");
  EXPECT_TRUE(p) << "Unable to get kp parameter.";
  double kp(1.0); //kp.setZero();
  ...
  double maxVelocity(10.0); // maxVelocity.setZero();
  ...
  EXPECT_TRUE(p->set("revolute1DoF_2"));
