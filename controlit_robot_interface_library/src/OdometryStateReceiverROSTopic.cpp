...
OdometryStateReceiverROSTopic::OdometryStateReceiverROSTopic() :
...
bool OdometryStateReceiverROSTopic::init(ros::NodeHandle & nh, RTControlModel * model)
{
    ...
    // Obtain the ROS topics for obtaining the odometry data
    std::string odometryTopic;

    if (!nh.getParam("controlit/odometry_topic", odometryTopic))
        odometryTopic = "robot_base_state";

    CONTROLIT_INFO << "Subscribing to the following odometry topic: " << odometryTopic;

    // Configure the reception of Odometry data
    // Subscribe to odometry topic.  The callback function is updateOdometry().
    PRINT_DEBUG_STATEMENT("Subscribing to odometry topic...");
    odomSubscriber = nh.subscribe(odometryTopic, 1, &OdometryStateReceiverROSTopic::odometryMessageCallback, this);
...
bool OdometryStateReceiverROSTopic::getOdometry(controlit::RobotState & latestRobotState, bool block)
{
...
void OdometryStateReceiverROSTopic::odometryMessageCallback(const boost::shared_ptr<nav_msgs::Odometry const>& odom)
{
